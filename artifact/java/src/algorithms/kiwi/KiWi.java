...
import algorithms.kiwi.ThreadData.PutData;
import algorithms.kiwi.ThreadData.ScanData;
...
{
	/*************** Constants ***************/
	public static int MAX_THREADS = 141;
	...
	public V get(K key)
	{
		...
		PutData<K,V> pd = null;
		pd = c.helpPutInGet((int) Camera.getTimestamp(), key);
	...
	public void put(K key, V val)
	{
		...
		while (true)
		{
			...
			// we need to help finish compact for its parent first, then proceed
			{
				...
				if (parent != null) {
					if (rebalance(parent) == null) {
						// System.out.println("ERROR: should never reach here"); // TODO email KIWI authors about this branch
						return;
					}
				}
			}
			...
			int myVersion = c.setVersion(oi, (int) Camera.getTimestamp());
...
*/
	...
	private int newVersion(K min, K max)
	{
		...
		int myVer = (int) Camera.takeSnapshot();
	...
	private TreeSet<Integer> getScans(int myVersion)
	{
		...
		for (int i = 0; i < MAX_THREADS; ++i)
		{
			...
			if (currScan.version.get() == Chunk.NONE)
			{
				...
				int ver = (int) Camera.takeSnapshot();
	...
	private Chunk<K,V> rebalance(Chunk<K,V> chunk)
	{
		...
		if(!rebalancer.isCompacted()) {
			ScanIndex<K> index = updateAndGetPendingScans((int) Camera.getTimestamp(), engaged);
