...
import java.util.concurrent.atomic.AtomicIntegerFieldUpdater;                // For atomic increments of counter
...
public class LockFreePBSTMap<K extends Comparable<? super K>, V> {
    ...
    */
    ...
    private static final AtomicIntegerFieldUpdater<LockFreePBSTMap> counterUpdater =
                         AtomicIntegerFieldUpdater.newUpdater(LockFreePBSTMap.class, "counter");
    ...
    */
    public final boolean containsKey(final K key) {
        ...
        while (true) {
            seq = counter;   // Update sequence number
    ...
    */
    public final Object[] rangeScan(final K a, final K b) {
        int seq = counterUpdater.getAndIncrement(this);  // Increment global version counter
    ...
    public final long rangeSum(final K a, final K b) {
        ...
        // Start the tree traversal
        return rangeSumHelper(root, seq, a, b);
    }
    ...
    */
    ...
    private final long rangeSumHelper(final Node<K,V> node, final int seq, final K a, final K b) {
        if (node.leftChild == null) {    // node is a leaf
            // In case the node's key falls between a and b, add it to the result of the range query
            if (node.key != null && a.compareTo(node.key) <= 0 && b.compareTo(node.key) >= 0) {
                return 1;
                // return (long) ((Integer) node.value);
            }
            return 0;
        }
        else {
            ...
            if (info.state == State.NULL || info.state == State.TRY)    // node participates in an in-progress operation
                help(info);
            if (node.key != null && a.compareTo(node.key) >= 0)           // node's key is below the lower limit of [a,b]
                return rangeSumHelper(readChild(node, false, seq), seq, a, b);  // traverse its right subtree
            else if (node.key == null || b.compareTo(node.key) < 0)       // node's key is above the upper limit of [a,b]
                return rangeSumHelper(readChild(node, true, seq), seq, a, b);   // traverse its left subtree
            else {
                // node is in [a,b] - traverse both of its subtrees
                return rangeSumHelper(readChild(node, false, seq), seq, a, b) + rangeSumHelper(readChild(node, true, seq), seq, a, b);
            }
        }
    }
    ...
    */
    private final boolean help(final Info<K,V> info) {
        ...
        if (info.state == State.NULL) {   // Optimization check
            if (counter != info.handshakingSeq)
