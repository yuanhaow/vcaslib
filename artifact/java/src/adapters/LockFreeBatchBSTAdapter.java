...
import algorithms.efrbbst.LockFreeBatchBSTMap;
...
public class LockFreeBatchBSTAdapter<K extends Comparable<? super K>> extends AbstractAdapter<K> implements SetInterface<K> {
    LockFreeBatchBSTMap<K,K> tree;

    public LockFreeBatchBSTAdapter(int k) {
        tree = new LockFreeBatchBSTMap<K,K>(k);
    }

    public LockFreeBatchBSTAdapter() {
        tree = new LockFreeBatchBSTMap<K,K>();
    }
